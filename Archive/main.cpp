    ...
    if (option==1){
        ...
        for (int i = 0; i < datavalues.size(); ++i) {
            if (vectest.empty()){
                ...
                bool hitTrue = false;
                count++;
                if (datavalues.at(i).setL == vectest.at(0).set) { //sets match
                    v.push_back(i);
                    for (int j = 0; j < vectest.size(); ++j) {
                        if (datavalues.at(i).tag == vectest.at(j).tag) {//hit
                            hit++;
                            hitTrue = true;
